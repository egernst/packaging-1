...
function setup_k3s() {
	...
	result=$(az vm create \
		...
		--name ${DNS_PREFIX}-control \
		--image UbuntuLTS \
		--nsg k3s-nsg \
		--admin-username kata \
		--ssh-key-values ${KEY_PATH}.pub )
	controlIP=$(echo $result | jq -r '.publicIpAddress')
	...
	echo control, worker: ${controlIP} ${workerIP}

	# Setup K3s on control/worker:
	curl -sLS https://raw.githubusercontent.com/alexellis/k3sup/master/get.sh | sh
	k3sup  install --ip $controlIP --user kata --ssh-key ${KEY_PATH}
	k3sup join --ip ${workerIP} --server-ip ${controlIP} --user kata --ssh-key ${KEY_PATH}
